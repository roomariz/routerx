...
jest.mock('../../src/commands/doctor/index.js', () => ({
  registerDoctorCommand: jest.fn()
}));

jest.mock('../../src/commands/init/index.js', () => ({
  registerInitCommand: jest.fn()
}));
...
describe('RegisterCommands', () => {
  ...
  const { registerDoctorCommand } = require('../../src/commands/doctor/index.js');
  const { registerInitCommand } = require('../../src/commands/init/index.js');
  ...
  describe('registerCommands', () => {
    test('registers all commands with the program', () => {
      ...
      expect(registerInitCommand).toHaveBeenCalledWith(mockProgram);
    ...
    test('calls each registration function exactly once', () => {
      ...
      expect(registerDoctorCommand).toHaveBeenCalledTimes(1);
      expect(registerInitCommand).toHaveBeenCalledTimes(1);
    ...
    test('registers commands with different program objects correctly', () => {
      ...
      expect(registerDoctorCommand).toHaveBeenCalledTimes(2);
      expect(registerInitCommand).toHaveBeenCalledTimes(2);
      ...
      expect(registerDoctorCommand).toHaveBeenCalledWith(mockProgram1);
      expect(registerDoctorCommand).toHaveBeenCalledWith(mockProgram2);
      expect(registerInitCommand).toHaveBeenCalledWith(mockProgram1);
      expect(registerInitCommand).toHaveBeenCalledWith(mockProgram2);
